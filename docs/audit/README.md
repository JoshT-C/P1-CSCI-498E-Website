...
Screenshots from the browser audit (`scripts/audit-browser.mjs`), kept
apart from `verification/`, which holds only the three items the brief
asks for.

**Produced by the audit (committed):**
- `profile-readme.txt` — the public GitHub profile README the About copy is derived
